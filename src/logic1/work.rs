...
use screeps_arena::{
    game::utils::{self, find_closest_by_range},
    prelude::*,
...
pub fn work() {
    ...
    let carrier = select_creeps::select_carriers();
    ...
    let mut full_containers_array = Array::new();
                ...
                if my_construction_sites.is_some() {
                    ...
                    if !worker.store().get_used_capacity(Some(ResourceType::Energy)) > 0 {
                        ...
                        if !containers_in_home.is_some() {
                            ...
                            if worker.withdraw(worker_container_in_home_closest.as_ref().unwrap().unchecked_ref(), Energy, Some(capacity)) == ReturnCode::NotInRange {
                                worker.move_to(worker_container_in_home_closest.as_ref().unwrap(), None);
                            }
                        }
                    } else {
                        ...
                        let worker_my_construction_sites_closest = worker.find_closest_by_range(my_construction_sites_array.as_ref());
                        if worker.build(worker_my_construction_sites_closest.as_ref().unwrap().unchecked_ref()) == ReturnCode::NotInRange {
                            worker.move_to(worker_my_construction_sites_closest.as_ref().unwrap(), None);
                        }
                    }
                } else {
                    if worker.store().get_free_capacity(Some(Energy)) > 0 {
                        let my_spawn_full_containers_closest = my_spawn.as_ref().unwrap().find_closest_by_range(full_containers_array.as_ref());
                        if worker.withdraw(my_spawn_full_containers_closest.as_ref().unwrap().unchecked_ref(), Energy, Some(worker.store().get_free_capacity(Some(Energy)) as u32)) == ReturnCode::NotInRange {
                            worker.move_to(my_spawn_full_containers_closest.as_ref().unwrap(), None);
                        }
                    } else {
                        if worker.transfer(my_spawn.as_ref().unwrap(), Energy, Some(worker.store().get_free_capacity(Some(Energy)) as u32)) == ReturnCode::NotInRange {
